    ...
    class DataGridViewColumnSelector
    {
        ...
        void mDataGridView_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right && e.RowIndex == -1)// && e.ColumnIndex == -1)
            {
                ...
                int PreferredHeight = (mCheckedListBox.Items.Count * 20) + 27;
                mCheckedListBox.Height = (PreferredHeight < MaxHeight) ? PreferredHeight : MaxHeight;
                mCheckedListBox.Width = this.Width;

                //new
                Point clickLocation = mDataGridView.GetCellDisplayRectangle(e.ColumnIndex, e.RowIndex, false).Location;
                mPopup.Show(mDataGridView.PointToScreen(new Point(clickLocation.X + e.X, clickLocation.Y + e.Y)));
